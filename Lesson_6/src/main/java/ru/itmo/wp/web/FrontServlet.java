...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route,
        ...
        Method meth = null;
        for (Class<?> clazz = pageClass; meth == null && clazz != null; clazz = clazz.getSuperclass()) {
            try {
                meth = clazz.getDeclaredMethod("before", HttpServletRequest.class, Map.class);
            }  catch (NoSuchMethodException ignored) {
                // No operations.
            }
        }
        applyMethod(request, pageClass, page, view, meth);
        ...
        for (Class<?> clazz = pageClass; meth == null && clazz != null; clazz = clazz.getSuperclass()) {
            try {
                meth = clazz.getDeclaredMethod("after", HttpServletRequest.class, Map.class);
            }  catch (NoSuchMethodException ignored) {
                // No operations.
            }
        }
    ...
    private void applyMethod(HttpServletRequest request, Class<?> pageClass, Object page, Map<String, Object> view, Method meth) throws ServletException {
        if (meth != null) {
            try {
