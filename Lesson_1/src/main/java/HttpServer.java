...
class HttpServer {
    ...
    private void process(Request request, Response response) throws IOException {
        ...
        File file = new File(root, URLDecoder.decode(request.getUri(), StandardCharsets.UTF_8));
        if (file.isDirectory()) {
        ...
        if (file.isFile()) {
            byte[] body;
            body = readFile(file);
            String expectedEtag = request.getHeader("If-None-Match");
            if (expectedEtag != null
                    && expectedEtag.equals(getEtag(body))) {
                response.setStatusCode(304);
                response.setHeader("ETag", expectedEtag);
            } else {
                ...
                String code = Integer.toString(Math.abs(Arrays.hashCode(body)));
                response.setHeader("ETag", code);
            }
        } else {
    ...
    private String getEtag(byte[] body) {
        return Integer.toString(Math.abs(Arrays.hashCode(body)));
    }
